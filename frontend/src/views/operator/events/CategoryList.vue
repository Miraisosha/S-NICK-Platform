...
const editingId = ref(null); // null = closed, 'new' = creating, otherwise category id
const defaultForm = () => ({
  name: '',
  gender: 'none',
  age_min: '',
...
async function submitForm() {
  ...
  try {
    ...
    if (editingId.value === 'new') {
      await categoriesApi.createCategory(eventId.value, payload);
    } else {
      await categoriesApi.updateCategory(eventId.value, editingId.value, payload);
    }
...
<template>
  <OperatorShell title="カテゴリ管理">
    ...
    <button
      ...
      type="button"
      class="auth-submit"
      style="max-width: 12rem; margin-bottom: 1.5rem"
      @click="openCreate"
    >
      カテゴリを新規登録
    </button>

    <form v-if="editingId !== null" class="operator-card" style="max-width: 44rem" @submit.prevent="submitForm">
      <h2>{{ editingId === 'new' ? 'カテゴリの新規登録' : 'カテゴリの編集' }}</h2>
    ...
    <div v-else class="operator-stat-grid">
      <div v-for="category in categories" :key="category.id" class="operator-card">
        ...
        <div style="display: flex; gap: 0.5rem">
          <button type="button" @click="openEdit(category)">編集</button>
          <button type="button" @click="onDelete(category)">削除</button>
        </div>
      </div>
    </div>
  </OperatorShell>
