...
タイトルを支援するスポンサーを登録・編集し、会場表示やライブ配信へ掲出する情報を管理します。

#### 登録項目

| 項目 | 内容 |
|---|---|
| 対象タイトル | スポンサーが支援するタイトル |
...
- 対象タイトルに登録されたスポンサーを一覧表示します。
- スポンサー名、ロゴ、表示状態、表示順を一覧で確認します。
- スポンサーの追加、編集、削除を行います。
- 表示順を並べ替えられるようにします。
...
画面下部のスポンサーロールでは、登録したロゴを途切れにくい形で横方向へ流します。スクロール速度、ロゴの高さ・間隔、背景色、表示順をタイトル単位で設定できる構成を候補とします。

スポンサー区分、表示回数、リンク先URL、掲載期間、固定表示するメインスポンサー、複数タイトル間でのスポンサー再利用、削除履歴、操作権限は検討中です。

### 4.9 スタッフ管理

ユーザー登録済みの人を、対象タイトルのスタッフとして関連付けます。スタッフはサイト全体の役割ではなく、招待・承諾したタイトルの範囲内でのみスタッフ権限を持ちます。

#### スタッフの追加

1. 運営がタイトルのスタッフ管理画面を開きます。
2. 登録済みユーザーをユーザーIDまたはメールアドレス等で指定します。
3. 担当業務と権限範囲を設定して招待します。
4. 対象ユーザーへメールまたはシステム内で招待を通知します。
5. 対象ユーザーが招待を承諾すると、タイトルのスタッフとして有効になります。
...
登録されていないメールアドレスを指定した場合に、新規ユーザー登録の案内を送るかは検討中です。第三者がユーザーを無制限に検索できないよう、検索結果へ表示する情報を制限します。

#### 管理項目

| 項目 | 内容 |
|---|---|
| 対象タイトル | スタッフとして参加するタイトル |
| 対象ユーザー | 登録済みユーザー |
| 担当業務 | 受付、試合進行、マーカー、配信等 |
| 担当コート | 担当するコート。全コートまたは複数選択を想定 |
| 権限 | 閲覧・更新可能な機能の範囲 |
...
担当業務の種類、複数担当、当日の時間帯・シフト管理は検討中です。

#### 権限の基本方針

- 運営は、担当するタイトルのスタッフを招待・編集・解除できます。
- スタッフは、招待を承諾したタイトルだけを管理画面に表示できます。
- スタッフは、対象タイトルへ申し込んだ参加者の一覧を閲覧できます。
- 住所、電話番号、緊急連絡先等は、担当業務上必要な権限が付与された場合だけ閲覧できます。
- 試合状態、マーカー、配信等の更新権限は、担当業務に応じて制限します。
- スタッフ権限からサイト全体のユーザー管理や、他タイトルの情報へアクセスすることはできません。
- 同じユーザーが、あるタイトルではスタッフ、別のタイトルでは選手または運営になることができます。

役割ごとの詳細な権限一覧と、運営がスタッフへどこまで権限を委譲できるかは検討中です。

#### 一覧・解除・履歴

- スタッフ名、担当業務、担当コート、権限、招待状態を一覧表示します。
- 招待の再送、取消、担当変更、スタッフ解除を行えます。
- スタッフを解除すると、そのタイトルへのスタッフ権限を直ちに無効化します。
- 招待、承諾、辞退、権限変更、解除の履歴を記録します。
- 過去の操作履歴を保つため、スタッフ関係を物理削除するかは検討中です。

## 5. コート管理

各地にあるスカッシュコート施設の詳細を、タイトルから共通利用できる施設マスターとして管理します。
...
- すでにタイトルや試合で使用されている施設を削除する場合は、関連データを確認します。

### 5.4 タイトル・試合との関係

タイトル管理の「場所・使用コート」では、コート管理へ登録された施設を一覧表示し、複数選択できるようにします。

```mermaid
flowchart LR
    Facility["コート施設"] --> Count["コート数"]
    Facility --> Title["タイトルの開催場所"]
    Count --> UsedCourts["タイトルで使用するコート"]
    UsedCourts --> Schedule["コート別時間割"]
    Schedule --> Match["試合"]
```

試合の自動時間割では、施設単位ではなくAコート、Bコート等の個別コートへ試合を割り当てる必要があります。施設登録時に個別コートを作成するか、タイトルごとに使用コート名を設定するかは検討中です。

営業時間、設備、連絡先、予約、施設管理者、コートごとの名称・特徴、配信・表示機器との対応は検討中です。
