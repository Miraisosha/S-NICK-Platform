...
最大ゲーム数は奇数とし、勝利に必要なゲーム数は `(最大ゲーム数 + 1) ÷ 2` で自動算出します。3ゲームマッチの場合は、左右各120秒のウォームアップ、120秒のインターバル、第1ゲーム、120秒のインターバル、第2ゲームと進み、1対1の場合だけ120秒のインターバル後に第3ゲームを行います。
...
表示は選手を行と列へ同じ順序で配置する対戦マトリクス形式とします。同一選手の対角セルと、重複する反対側のセルは無効表示とします。試合セルには試合番号、コート、開始予定時刻を表示し、終了後は勝敗、ゲームカウント、各ゲーム得点を表示します。マトリクス右端には「対戦なし」を除いた各選手の有効な試合数を自動計算して常時表示し、対戦の除外・再有効化時に即時更新します。勝敗、得失ゲーム、得失点、順位等のその他の集計列も右端に表示する候補とします。
...
フェーズ2の自動生成結果は下書きとして表示し、運営者が確認・手動調整してから公開します。公開後はパブリック日程、フェーズ3の選手マイページ、試合状況モニターへ反映します。
...
試合が未開始のまま、現在有効な開始予定時刻から5分以上経過した場合に「遅延」と判定します。判定にはS-NICKサーバー時刻を使用し、開始予定時刻を変更した場合は変更後の時刻を基準にします。開始済み、終了済み、棄権、失格、中止の試合は、新たな遅延判定の対象外です。

遅延時間は現在有効な開始予定時刻から実際の開始時刻までとし、未開始中はS-NICKサーバーの現在時刻までの時間を表示します。
...
現在時刻と赤い現在時刻線は表示端末の時計ではなく、S-NICKサーバー時刻を基準にします。試合状況モニター用APIのレスポンスへ、レスポンス生成時点のサーバー時刻をタイムゾーン判別可能なISO 8601形式で含めます。
...
TVモニター、タブレット、PC等は共通の閲覧専用表示端末として扱い、大会・コートごとの長いURLやログイン情報を入力せず、固定表示ページと端末ペアリングを使用します。試合状況一覧で6コート以上を対象とする場合は、表示設定で10秒ごとのコートグループ自動切替、またはモニターごとのコート分割を選択します。自動切替ではコートの表示順を設定し、モニター分割では各端末へ最大5コートを割り当てます。

TV、PC、HDMI接続端末では、未登録端末の固定表示ページへ10分間有効・1回限りの6桁コードとQRコードを表示し、運営者がスマートフォンまたはPCからイベント、開催日、コート、表示種別を割り当てます。タブレットでは、運営画面で割当先を選択して発行した10分間有効・1回限りのQRコードを標準カメラで読み取ります。QRコードを使用できない場合は6桁コードを代替手段とします。

割当完了後は端末を自動的に対象表示へ切り替え、使用済みの接続情報を直ちに無効化します。

接続後は閲覧専用の端末キーを保存し、次回起動時に設定を復元します。運営者は端末名、最終接続、割当先、接続状態を確認し、端末を再接続させず遠隔で割当変更・解除・無効化できます。タブレットは横画面・全画面表示を基本とし、必要に応じて固定表示ページをホーム画面へ追加します。TVブラウザが運用に適さない場合は、固定表示ページを自動起動するHDMI接続の小型ブラウザ端末を使用します。

表示URLの公開範囲、アクセス制御、画面解像度、縦横の向き、文字サイズ、配色、音声による呼出、複数言語対応は検討中です。

#### コート別大型得点ボード

試合状況モニターとは別に、対象コートで現在行われている試合の得点を大きく表示する、タブレット・TVモニター・PC向けの閲覧専用画面を提供します。共通の端末ペアリングで表示対象コートを割り当て、ブラウザの全画面表示で使用します。
...
この図は要件整理のための概念図であり、テーブル設計ではありません。イベント所有者とスタッフは個人の共通アカウントに紐づきます。団体所有と団体戦は将来検討とします。
