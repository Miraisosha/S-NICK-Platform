...
本書は、要件から論理テーブル設計へ進むための概念ER設計である。エンティティ名と主要項目は論理設計の候補であり、MySQLの物理テーブル名、型、桁数、インデックスおよびマイグレーションは詳細設計で確定する。データベースの技術・運用方針は[データベース](201_database.md)、業務概念の概要は[データモデル](203_model.md)を参照する。

- 確認時点：2026年8月7日
- 対象：フェーズ1の大会運営・マーカー・大型表示を中心とし、グループ戦、OBS、多言語等の拡張点を含む
- 正式な試合状態コード：[`state-machine.yaml`](../specifications/yaml/state-machine.yaml)
- 画面との対応：[フロントエンド](301_frontend.md)
...
- [データモデル](203_model.md)
- [利用者・権限](../specifications/020_UserRoles.md)
- [運営者仕様](../specifications/200_Operator.md)
- [マーカー仕様](../specifications/300_Marker.md)
- [共通基盤](../specifications/600_CommonPlatform.md)
