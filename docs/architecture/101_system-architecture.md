...
| フロントエンド | Vue.js 3.5 / Vite 8 / 独自CSS。Bootstrap 5.3は移行期間中の既存CakePHP画面に限定 |
| データベース | MySQL |
...
日時は、サーバーOSのタイムゾーンに依存させず、次の方針で扱う。

| 対象 | 方針 | 状態 |
|---|---|---|
| CakePHPの内部処理 | UTCを使用する | 決定済み |
| MySQL接続 | セッションタイムゾーンをUTCにする | 決定済み |
| データベース保存 | 発生時刻や予定日時等の時点を表す値はUTCで保存する | 決定済み |
| イベントのタイムゾーン | 開催地のIANAタイムゾーン名を`events.timezone`へ保持する | 決定済み |
| イベント作成時の初期値 | ブラウザのタイムゾーンを使用し、取得できない場合は`Asia/Tokyo`とする | 決定済み |
| 日時の入出力 | イベントに関する日時は、保存済みのイベントタイムゾーンへ変換する | 決定済み |

ブラウザのタイムゾーンはイベント作成時の初期値に限定し、運営者が確認・変更できるようにする。イベント作成後は、アクセスした端末のタイムゾーンによって`events.timezone`を自動変更しない。これにより、運営者が異なる国や地域からアクセスした場合も、大会日程は開催地の時間を基準に扱う。

個人向けの操作履歴や通知日時は、将来ユーザー別タイムゾーン設定を追加した場合はその設定を優先し、未設定時にブラウザのタイムゾーンを使用する。システムログおよび監査ログはUTCを基準とする。生年月日や開催日のように時刻を含まない値は`DATE`として扱い、UTC変換の対象にしない。

現在はCakePHPの基底設定とCIのデータベース接続がUTCである一方、Docker Composeのアプリケーション、MySQLおよびデータベース接続は`Asia/Tokyo`である。実装時にUTCへ統一し、既存日時データへの影響を確認する。データベースでの型と変換境界の詳細は[データベース](201_database.md)で管理する。
